...
//        soundPool.play(1, 1, 1, 0, 0, 1);
     ...
     * 消耗资源较小
     */
    public static void playSoundArray(Context context, Integer... itgr) {
        SoundPool soundPool;
        final HashMap<Integer,Integer> sound = new HashMap<>();
        if (Build.VERSION.SDK_INT >= 21) {
            SoundPool.Builder builder = new SoundPool.Builder();
            //传入音频的数量
            builder.setMaxStreams(itgr.length);
            //AudioAttributes是一个封装音频各种属性的类
            AudioAttributes.Builder attrBuilder = new AudioAttributes.Builder();
            //设置音频流的合适属性
            attrBuilder.setLegacyStreamType(AudioManager.STREAM_MUSIC);
            ...
            //第一个参数是可以支持的声音数量，第二个是声音类型，第三个是声音品质
            soundPool = new SoundPool(itgr.length, AudioManager.STREAM_SYSTEM, 5);
        }
        //第一个参数Context,第二个参数资源Id，第三个参数优先级
        for (int rawId:itgr ) {
            int soundId = soundPool.load(context, rawId, 1);
            sound.put(rawId,soundId);
        }
        ...
        soundPool.setOnLoadCompleteListener(new SoundPool.OnLoadCompleteListener() {
            ...
            public void onLoadComplete(SoundPool soundPool, int sampleId, int status) {
                int i = 0;
                for (int sdId:sound.values() ) {
                    soundPool.play(sdId, 1, 1, i++, 0, 1);
                }
//                soundPool.play(1, 1, 1, 0, 0, 1);
            }
        });
